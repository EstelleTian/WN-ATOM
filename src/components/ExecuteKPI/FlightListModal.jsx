...
function FlightListModal(props) {
    ...
    const {flightListModalVisible, getFocusCategoryFlight, flightListCategoryZh} = executeKPIData;
    ...
    const columns = getColumns(commonSubNames)
    //转换为表格数据
    let tableData = getFocusCategoryFlight.map( flight => formatSingleFlight(flight) );
    ...
    return (
        <Spin spinning={false} >
            <DraggableModal
                title={(<span>{flightListCategoryZh}—{tacticName} </span>)}
...
export default inject("executeKPIData","performanceKPIData","schemeListData")(observer(FlightListModal));