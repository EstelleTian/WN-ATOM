 ...
 * @Author: liutianjiao
 * @Date:
 * @LastEditTime: 2021-04-27 16:44:50
...
const DetailBtn = function (props) {
  ...
  return (
    <span>
      <Link to={`/workflow_detail/list/${props.record.sid}`} target="_blank">
        详情
      </Link>
    </span>
  );
...
function WorkFlowList(props) {
  ...
  const finishedColumns = [
    ...
    },
    {
      ...
      render: (text, record, index) => {
        ...
        return <span style={{ color: textClass }}>{text}</span>;
      },
    },
    {
      title: "工作所处环节",
  ...
  //处理 办结工作 数据
  const handleTasksData = useCallback(
    (data) => {
  ...
  return (
    <div className="work_cont">
      ...
      <div className="date_time">
        数据时间：{formatTimeString(workFlowData.generateTime)}
      </div>
    </div>
  );
}
...
export default withRouter(
  inject("workFlowData", "systemPage")(observer(WorkFlowList))
);
