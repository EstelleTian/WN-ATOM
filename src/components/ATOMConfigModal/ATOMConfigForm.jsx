...
function ATOMConfigForm(props) {
    const { systemPage, ATOMConfigFormData = {} } = props;
    // 用户
    ...
    // 配置选项数据
    const configOptionData = ATOMConfigFormData.configOptionData || {};
    // 选项数据格式化为数组
    const optionData = Object.keys(configOptionData);
    // 当前系统选中的配置值
    const configValue = ATOMConfigFormData.configValue || "";
  ...
  const submitData = (value) => {
    const opt = {
      url:
        ...
        "/update/atomDataApplySwitch/" +
        value +
        "?username=" +
        userName,
      method: "POST",
      params: {},
      resFunc: (data) => requestSuccess(data, value),
    ...
    const submitData = (value) => {
        const opt = {
            url: ReqUrls.switchConfigUrl + "/update/atomDataApplySwitch/" + value + "?username=" + userName,
            method: "POST",
            params: {},
  ...
  //数据提交成功回调
  const requestSuccess = useCallback((data, value) => {
    Modal.success({
      title: "保存成功",
      content: (
        <span>
      ...
      okText: "确定",
      onOk: () => {
        ATOMConfigFormData.configValue = value;
        flightTableData.atomConfigValue = value;
...
export default inject("systemPage", "ATOMConfigFormData")(observer(ATOMConfigForm));
