...
function FlightExchangeSlotForm(props) {
    ...
    // 申请航班数据
    const claimantFlightCoordinationData = flightExchangeSlotFormData.claimantFlightCoordinationData || {};
    // 申请航班格式化后的信息
    ...
    const handleSubmitData = (values) => {
        ...
        //目标航班对象
        let targetFlightData = alterTargetFlightList.find(item => item.id == values.targetFlight);
        // 目标航班信息
                    ...
                    <Col span={24}>
                        ...
                        >
                            <Select
                                ...
                                notFoundContent={fetching ? <Spin size="small" /> : "无"+searchKey+"相关可交换时隙航班数据"}
                                placeholder="请输入航班号"
                            >
                                {options}
                            </Select>
...
export default inject("flightTableData", "schemeListData", "flightExchangeSlotFormData", "systemPage")(observer(FlightExchangeSlotForm));
