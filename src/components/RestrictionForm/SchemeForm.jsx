...
import TacticIntervalDepApForm from "./TacticIntervalDepApForm";
import TacticIntervalArrApForm from "./TacticIntervalArrApForm";
...
import {
  handleImportControl,
  handleImportControlForUpdate,
  handleUpdateFlowControl,
  handleCreateSchemeBySimulation,
} from "utils/client";
import {
  parseFullTime,
  isValidObject,
...
function SchemeForm(props) {
  // 可显示快捷录入表单的页面类型集合
  const isShowShortcutFormPageType =
    SchemeFormUtil.getIsShowShortcutFormPageType();
  // 原航路表单字段配置
  let [originRouteField, setOriginRouteField] = useState(
    SchemeFormUtil.InitialOriginRouteFieldData
  );

  // 备选航路表单字段配置集合
  let [alterRoutesField, setAlterRoutesField] = useState(
    SchemeFormUtil.InitialAlterRoutesFieldData
  );
  // 主按钮是否禁用
  let [primaryBtnDisabled, setPrimaryBtnDisabled] = useState(false);
  // 另存为模板框态框是否显示
  let [saveAsTemplateModalVisible, setSaveAsTemplateModalVisible] =
    useState(false);
  const {
    systemPage,
    schemeFormData,
  ...
  // 方案半数起飞机场表单
  const [tacticIntervalDepApForm] = Form.useForm();
  // 方案半数降落机场表单
  const [tacticIntervalArrApForm] = Form.useForm();
  // 方案高度表单
  ...
  const getAllFieldsValue = () => {
    ...
    if (
      ...
      promiseArray = [...promiseArray, tacticReserveSlotFormValidatePromise()];
    }
    return Promise.all(promiseArray);
  };
  ...
  const getSaveAsTemplateNecessaryFieldsValue = () => {
    let promiseArray = [tacticMeasureFormValidatePromise()];
    if (restrictionMode === "CR") {
      promiseArray = [
        ...promiseArray,
        tacticOriginRouteFormValidatePromise(),
  ...
  const spliceName = (fieldData) => {
    ...
    // 豁免后序单元
    exemptBehindUnit = isValidVariable(exemptBehindUnit)
      ? exemptBehindUnit.toUpperCase()
      : "";
    // 限制数值单位
    ...
    // 降落机场
    arrAp =
      isValidVariable(arrAp) && isValidVariable(arrAp.join(";"))
        ? arrAp.join(";").toUpperCase()
        : "";
    // 豁免降落机场
    exemptArrAp =
      isValidVariable(exemptArrAp) && isValidVariable(exemptArrAp.join(";"))
        ? exemptArrAp.join(";").toUpperCase()
        : "";
    // 快捷录入勾选中的复选框
    shortcutInputCheckboxSet =
      isValidVariable(shortcutInputCheckboxSet) &&
      isValidVariable(shortcutInputCheckboxSet.join(";"))
        ? shortcutInputCheckboxSet.join(";").toUpperCase()
        : "";
    let behindUnitLabel = isValidVariable(behindUnit)
      ? `去往${behindUnit} `
      : "";
    let exemptBehindUnitLabel = isValidVariable(exemptBehindUnit)
      ? `(${exemptBehindUnit}除外), `
      : "";
    let arrApLabel = isValidVariable(arrAp) ? `落地(${arrAp}) ` : "";
    let exemptArrApLabel = isValidVariable(exemptArrAp)
      ? `(${exemptArrAp}除外) `
      : "";
  ...
  const tacticDateTimeFormValidate = async (resolve, reject) => {
    try {
      // 必要校验字段
      const fields = ["startDate", "endDate", "startTime", "endTime"];
      // 触发表单验证取表单数据
  ...
  const tacticShortcutInputFormValidate = async (resolve, reject) => {
    try {
      // 必要校验字段
      const fields = ["shortcutInputCheckboxSet"];
      // 触发表单验证取表单数据
  ...
  const getRouteValueParams = () => {
    ...
    for (
      let i = 0;
      i < SchemeFormUtil.InitialAlterRoutesFieldData.length;
      i++
    ) {
      let val = tacticAlterRouteForm.getFieldValue(
        SchemeFormUtil.InitialAlterRoutesFieldData[i].name
      );
  ...
  const updateSingleAlterRouteData = (item, validateResult) => {
    ...
    let data = null;
    if (
      isValidVariable(validateResult) &&
      ...
      data =
        validateResult.find(
          (element) => `alterRoute${element.paramIndex}` === name
        ) || {};
    }
    // 返回值
    let obj = {
    ...
    if (isValidObject(data)) {
      ...
      if (data.correct) {
        ...
        // 反之,此表单未校验通过
        tacticAlterRouteForm.setFields([
          { name: name, errors: [`${data.errorReason}`] },
        ]);
      ...
      return new Promise((resolve, reject) => {
        ...
        const opt = {
          ...
          resFunc: (data) => {
            ...
            if (
              isValidObject(data) &&
              Array.isArray(data.alterRoutes) &&
              data.alterRoutes.length > 0
            ) {
              ...
              let routeValidate =
                validateResult.find(
                  (element) => `alterRoute${element.paramIndex}` === field
                ) || {};
  ...
  const getTitle = () => {
    if (
      restrictionMode != "CR" &&
      isShowShortcutFormPageType.includes(pageType)
    ) {
      // 除改航限制类型外的且操作为可显示快捷录入的
      return (
        <Space size={20}>
          交通流信息{" "}
          <TacticInputMethodRadioGroup disabledForm={props.disabledForm} />
        </Space>
      );
    } else {
      return <Space size={20}>交通流信息</Space>;
    }
  };
  ...
  const drawSaveAsTemplateBtn = () => {
    return (
      <div className="button-container">
        <Button type="primary" onClick={handleSaveAsTemplateBtnClick}>
          另存为模板
        </Button>
      </div>
    );
  ...
  const drawOperation = () => {
    return (
      <div
        className={
          props.operationBarClassName
            ? `operation-bar ${props.operationBarClassName}`
            : "operation-bar"
        }
      >
        <Space>
   ...
   * */
  const drawEditBtn = () => {
    return props.disabledForm ? (
      <div className="button-container">
        <Button className="btn_edit" type="primary" onClick={handleOpenEdit}>
          编辑
        </Button>
      </div>
    ) : (
  ...
  const validateStartDateTimeRange = () => {
    ...
    if (
      ...
      parseFullTime(startDateTimeString).getTime()
    ) {
      return false;
    } else {
      return true;
  ...
  const checkTacticName = (fieldData, skipConfirm) => {
    ...
    if (isValid) {
      ...
      Modal.confirm({
        ...
        content: (
          <div>
            <p>{`限制条件与方案名称不相符，建议自动命名`}</p>
          </div>
        ),
        okText: `自动命名并提交`,
        cancelText: `提交`,
        onOk: () => {
          handleAutoFillTacticNameSubmitFormData(fieldData);
        },
        onCancel: (close) => {
          // 若close为函数则为取消按钮点击触发，反之为关闭按钮触发
          if (typeof close === "function") {
  ...
  const submitData = () => {
    ...
    if (
      ...
      if (isValidVariable(clearSimIdFlag)) {
        ...
        opt.url =
          ReqUrls.modifySimulationSchemeUrl + user.id + "/" + clearSimIdFlag;
      } else {
        opt.url = ReqUrls.modifySimulationSchemeUrl + user.id;
      }
      opt.method = "PUT";
    } else if (
      pageType === SchemeFormUtil.PAGETYPE_IMPORT ||
  ...
  const handleFormData = () => {
    ...
    // 更新半数起飞机场
    flowControlMeasure.intervalDepFlight = formDataValue.intervalDepFlight;
    // 更新半数降落机场
    flowControlMeasure.intervalArrFlight = formDataValue.intervalArrFlight;
    // 改航限制相关数值更新
    if (restrictionMode === "CR") {
      // 更新原航路数据信息
  ...
  const getDirectionListData = (formDataValue) => {
    ...
    if (inputMethod === SchemeFormUtil.INPUTMETHOD_CUSTOM) {
      ...
      // 快捷录入模式取值
      directionData =
        getSingleDirectionListDataByShortcutFormSelecedData(formDataValue);
    }
    return directionData;
  };
  // 快捷录入模式获取方案方向信息集合数据(单方向)
  const getSingleDirectionListDataByShortcutFormSelecedData = (
    formDataValue
  ) => {
    const { useHeight, exemptHeight, depAp, arrAp, exemptDepAp, exemptArrAp } =
      formDataValue;
    // 基准单元
    let targetUnit = [];
    // 前序单元
  ...
  const getMultipleDirectionListDataByShortcutFormSelecedData = (
    formDataValue
  ) => {
    let list = [];
    const { useHeight, exemptHeight, depAp, arrAp, exemptDepAp, exemptArrAp } =
  ...
  const getAllFormValue = () => {
    ...
    // 限制原因
    const flowControlReason =
      tacticReasonForm.getFieldValue("flowControlReason");
    ...
    // AFP限制数值
    const restrictionAFPValueSequence = tacticMeasureForm.getFieldValue(
      "restrictionAFPValueSequence"
    );
    // TC限制分钟数值
    const restrictionTCPeriodDuration = tacticMeasureForm.getFieldValue(
      "restrictionTCPeriodDuration"
    );
    // TC限制架次数值
    const restrictionTCPeriodValue = tacticMeasureForm.getFieldValue(
      "restrictionTCPeriodValue"
    );
    // MIT限制数值
    const restrictionMITValue = tacticMeasureForm.getFieldValue(
      "restrictionMITValue"
    );
    // MIT时间间隔字段数值
    const restrictionMITTimeValue = tacticMeasureForm.getFieldValue(
      "restrictionMITTimeValue"
    );
    // 基准单元
    let targetUnit = "";
    ...
    if (
      restrictionMode !== "CR" &&
      inputMethod === SchemeFormUtil.INPUTMETHOD_CUSTOM
    ) {
      // 更新基准单元数值
      ...
      // 更新豁免前序数值
      exemptFormerUnit =
        tacticExemptFormerUnitForm.getFieldValue("exemptFormerUnit");
      // 更新豁免后序数值
      exemptBehindUnit =
        tacticExemptBehindUnitForm.getFieldValue("exemptBehindUnit");
    }
    ...
    // 半数起飞机场
    const intervalDepFlight =
      tacticIntervalDepApForm.getFieldValue("intervalDepFlight");
    // 半数降落机场
    const intervalArrFlight =
      tacticIntervalArrApForm.getFieldValue("intervalArrFlight");
    ...
    // 包含-尾流类型
    const wakeFlowLevel =
      tacticLimitedFlightForm.getFieldValue("wakeFlowLevel");
    // 包含-运营人
    ...
    // 不包含-航班号
    const exemptionFlightId =
      tacticExemptFlightForm.getFieldValue("exemptionFlightId");
    // 不包含-尾流类型
    const exemptionWakeFlowLevel = tacticExemptFlightForm.getFieldValue(
      "exemptionWakeFlowLevel"
    );
    // 不包含-航班类型
    const exemptionAirlineType = tacticExemptFlightForm.getFieldValue(
      "exemptionAirlineType"
    );
    // 不包含-运营人
    const exemptionAuType =
      tacticExemptFlightForm.getFieldValue("exemptionAuType");
    // 不包含-客货类型
    const exemptionMissionType = tacticExemptFlightForm.getFieldValue(
      "exemptionMissionType"
    );
    // 不包含-任务类型
    const exemptionTask = tacticExemptFlightForm.getFieldValue("exemptionTask");
    // 不包含-军民航
    const exemptionOrganization = tacticExemptFlightForm.getFieldValue(
      "exemptionOrganization"
    );
    // 不包含-限制资质
    const exemptionAbility =
      tacticExemptFlightForm.getFieldValue("exemptionAbility");
    // 不包含-受控机型
    const exemptionAircraftType = tacticExemptFlightForm.getFieldValue(
      "exemptionAircraftType"
    );
    ...
    let result = {
      ...
      restrictionMITTimeValue,
      // 起飞机场
      depAp: SchemeFormUtil.parseAreaLabelAirport(
        depAp,
        areaAirportListData
      ).join(";"),
      // 降落机场
      arrAp: SchemeFormUtil.parseAreaLabelAirport(
        arrAp,
        areaAirportListData
      ).join(";"),
      // 豁免起飞机场
      exemptDepAp: SchemeFormUtil.parseAreaLabelAirport(
        exemptDepAp,
        areaAirportListData
      ).join(";"),
      // 豁免降落机场
      exemptArrAp: SchemeFormUtil.parseAreaLabelAirport(
        exemptArrAp,
        areaAirportListData
      ).join(";"),
      // 半数起飞机场
      intervalDepFlight: SchemeFormUtil.parseAreaLabelAirport(
        intervalDepFlight,
        areaAirportListData
      ).join(";"),
      // 半数降落机场
      intervalArrFlight: SchemeFormUtil.parseAreaLabelAirport(
        intervalArrFlight,
        areaAirportListData
      ).join(";"),
  ...
  const requestSuccess = (oldId, data) => {
    ...
    } else if (pageType === SchemeFormUtil.PAGETYPE_MODIFY) {
      // 正式方案进行模拟修改操作
    ...
    } else if (pageType === SchemeFormUtil.PAGETYPE_IMPORTWITHFORMER) {
      //  外区流控导入方案，且已有关联的方案
  ...
  const getTargetUnit = () => {
    ...
    if (
      restrictionMode !== "CR" &&
      inputMethod === SchemeFormUtil.INPUTMETHOD_SHORTCUT
    ) {
  ...
  const updateMITTimeValueChange = async () => {
    try {
      // 必要校验字段
      const fields = ["restrictionMITValue"];
      // 触发表单验证取表单数据
      const fieldData = await tacticMeasureForm.validateFields(fields);
      const restrictionMITValue = tacticMeasureForm.getFieldValue(
        "restrictionMITValue"
      ...
      if (
        ...
        if (restrictionMITValueUnit === "T") {
          tacticMeasureForm.setFieldsValue({
            restrictionMITTimeValue: restrictionMITValue,
          });
        } else if (restrictionMITValueUnit === "D") {
          let newValue = "";
          if (
            isValidVariable(restrictionMITValue) &&
            isValidVariable(distanceToTime)
          ) {
            newValue = Math.ceil(
              parseInt(restrictionMITValue, 10) / parseInt(distanceToTime, 10)
            );
          }
          tacticMeasureForm.setFieldsValue({
            restrictionMITTimeValue: newValue,
          });
        }
      }
    } catch (errorInfo) {
  ...
  useEffect(
    function () {
      // 若MIT限制类型单位为距离
      if (
        isValidVariable(restrictionMITValueUnit) &&
        restrictionMITValueUnit === "D"
      ) {
        // 更新MIT时间间隔字段数值
        updateMITTimeValueChange();
      }
    ...
    <div className="scheme-form-container">
      ...
      <Card title="方案信息" bordered={false} size="">
        ...
        <Row className="info-row">
          ...
          {props.disabledForm ? (
            ...
            <Col span={3}>
              <Form.Item>
                <Button type="primary" onClick={autofillTacticName}>
                  自动命名
                </Button>
              </Form.Item>
            </Col>
      ...
      <Card title="措施信息" bordered={false} size="">
        {pageType === SchemeFormUtil.PAGETYPE_CREATE && (
          <Row className="info-row">
            <Col span={20}>
              <TacticTemplateForm disabledForm={props.disabledForm} />
            </Col>
          </Row>
        )}
        <TacticMeasureForm
        ...
        <Row className="info-row">
          {/* 半数起飞机场 */}
          <Col span={12}>
            <TacticIntervalDepApForm
              pageType={pageType}
              form={tacticIntervalDepApForm}
              disabledForm={props.disabledForm}
            />
          </Col>
          {/* 半数降落机场 */}
          <Col span={12}>
            <TacticIntervalArrApForm
              pageType={pageType}
              form={tacticIntervalArrApForm}
              disabledForm={props.disabledForm}
            />
          </Col>
        </Row>
      ...
      <Card title={getTitle()} bordered={false} className="flow-control-flight">
        ...
        <Row className="info-row">
          <Col span={12}>
            <TacticLimitedFlightForm
              title="包含"
              pageType={pageType}
              form={tacticLimitedFlightForm}
              disabledForm={props.disabledForm}
            />
          </Col>
          <Col span={12}>
            <TacticExemptFlightForm
              title="不包含"
              pageType={pageType}
              form={tacticExemptFlightForm}
              disabledForm={props.disabledForm}
            />
          </Col>
        </Row>
        ...
        >
          <Row className="info-row">
            <Col span={24}>
              <TacticTemplateNameForm form={tacticTemplateNameForm} />
            </Col>
          </Row>
        </Modal>
      </Card>
