...
//方案半数降落机场表单
function TacticIntervalArrApForm(props) {
  ...
  useEffect(
    ...
    [tacticName, intervalArrFlight]
  );

  // 自定义 tag 内容 render
  const tagRender = ({ label, closable, onClose, value }) => {
    // 过滤出当前录入的标签下的机场集合
    let airport = SchemeFormUtil.filterAreadLabelAirport(
      value,
      areaAirportListData
    );
    return (
  ...
  const shortcutInputValue = (label) => {
    // 获取当前字段值
    let fieldValue = form.getFieldValue("intervalArrFlight");
    ...
    // 更新表单数值
    form.setFieldsValue({ intervalArrFlight: valueArr });
  };
  // 处理起飞机场数值
  const handleValue = (val) => {
    // 处理机场数值
    let newValue = SchemeFormUtil.handleAirportSelectInputValue(
      val,
      areaAirportListData
    );
    // 更新表单数值
    form.setFieldsValue({ intervalArrFlight: newValue });
  };
  // 绘制单个区域按钮
  const drawSingleSpecialAreaAirport = (area) => {
    const { provinceName, provinceNameZH, value } = area;
    return (
      <Tooltip placement="bottom" title={value} key={provinceName}>
        <Button
          size="small"
          onClick={() => {
            shortcutInputValue(provinceNameZH);
          }}
        >
          {provinceNameZH}
        </Button>
...
export default inject("schemeFormData")(observer(TacticIntervalArrApForm));
