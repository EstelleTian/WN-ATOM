...
function TacticArrApForm(props) {
    ...
    // 方案降落机场
    let arrAp = isValidVariable(schemeFormData.arrAp) ? schemeFormData.arrAp.split(';') : [];
    // 格式化后的降落机场数值
    let formatArrAp = SchemeFormUtil.formatAreaLabel(arrAp, areaAirportListData);
    // 是否为禁用页面类型
    ...
    */
    ...
    const shortcutInputValue = (label) => {
        // 获取当前字段值
        let fieldValue = form.getFieldValue('arrAp');
...
export default inject("schemeFormData",)(observer(TacticArrApForm));
