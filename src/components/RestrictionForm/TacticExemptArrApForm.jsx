...
//方案豁免降落机场表单
function TacticExemptArrApForm(props) {
    ...
    let exemptArrAp = isValidVariable(schemeFormData.exemptArrAp) ? schemeFormData.exemptArrAp.split(';') : [];
    // 格式化后的豁免降落机场数值
    let formatExemptArrAp = SchemeFormUtil.formatAreaLabel(exemptArrAp, areaAirportListData);
    // 是否为禁用页面类型
    ...
    */
    ...
    const shortcutInputValue = (label) => {
        // 获取当前字段值
        let fieldValue = form.getFieldValue('exemptArrAp');
        ...
        // 更新表单数值
        form.setFieldsValue({ 'exemptArrAp': valueArr });
    }
    // 处理起飞机场数值
    const handleValue = (val) => {
        ...
        // 更新表单数值
        form.setFieldsValue({ 'exemptArrAp': newValue });
    }
    // 绘制单个区域按钮
...
export default inject("schemeFormData",)(observer(TacticExemptArrApForm));
