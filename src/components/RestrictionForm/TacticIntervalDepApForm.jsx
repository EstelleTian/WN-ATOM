...
function tacticIntervalDepApForm(props) {
  ...
  let intervalDepFlight = isValidVariable(flowControlMeasure.intervalDepFlight)
    ? flowControlMeasure.intervalDepFlight.split(";")
    : [];
  // 格式化后的半数起飞机场数值
  let formatIntervalDepAp = SchemeFormUtil.formatAreaLabel(
    intervalDepFlight,
    areaAirportListData
  );
  // 是否为禁用页面类型
