...
// import './RunwayDynamicEditForm.scss'


// 动态跑道修改表单
function RunwayDynamicEditForm(props) {
    ...
    const { systemPage, RunwayDynamicEditFormData, runwayListData } = props;
    // 用户id
    ...
    // loading
    const loading = RunwayDynamicEditFormData.loading;
    // 跑道状态勾选数值变更计数器
    const ruwayStatusSelectedDataChangeCounter = RunwayDynamicEditFormData.ruwayStatusSelectedDataChangeCounter;
    // 跑道航路点勾选数值变更计数器
    const runwayPointSelectedDataChangeCounter = RunwayDynamicEditFormData.runwayPointSelectedDataChangeCounter;

    // 获取数据时的请求参数
    const requestParams = RunwayDynamicEditFormData.requestParams || {};
    // 配置数据
    const configData = RunwayDynamicEditFormData.configData || {};
    // 机场名称
    const apName = RunwayDynamicEditFormData.airport || "";
    // 运行模式
    const operationmode = RunwayDynamicEditFormData.operationmode || "";
    // 各跑道状态勾选数值
    const ruwayStatusSelectedData = RunwayDynamicEditFormData.ruwayStatusSelectedData || {};
    // 各跑道航路点勾选数值
    const runwayPointSelectedData = RunwayDynamicEditFormData.runwayPointSelectedData || {};
    // 跑道数量
    const runwayNumber = RunwayDynamicEditFormData.runwayNumber || 0;

    // 终端当前时间
    let now = moment();
    const groupId = RunwayDynamicEditFormData.groupId || ""
    // 开始时间
    const startTime = RunwayDynamicEditFormData.startTime || "";
    // 结束时间
    const endTime = RunwayDynamicEditFormData.endTime || "";
    ...
    // 跑道航路点
    const runwayPoint = RunwayDynamicEditFormData.runwayPoint || [];
    // 跑道配置数据
    const listRWGapInfo = RunwayDynamicEditFormData.listRWGapInfo || [];
    // 获取跑道配置字段及数值
    ...
    * */
        ...
        return (
            <Fragment key={runwayData.id}>
                <RunwaySingleConfigDataForm
                    ...
                    storesData={RunwayDynamicEditFormData}
                >
                </RunwaySingleConfigDataForm>
            </Fragment>)
    }
    ...
    const getUnSelectedPoint = () => {
        // 所有航路点数据
        let allPoint = [...RunwayDynamicEditFormData.allPoint];
        // 所有跑道勾选的航路点数据
        let selectedPoint = [];
    ...
    const handleSubmitData = (values) => {
        ...
        for (let i = 0; i < listRWGapInfo.length; i++) {
            ...
            // 注意：字段使用listRWGapInfoDynamic前缀
            let obj = {
                [`listRWGapInfoDynamic[${i}].id`]: id,
                [`listRWGapInfoDynamic[${i}].apName`]: apName,
                [`listRWGapInfoDynamic[${i}].rwName`]: name,
                [`listRWGapInfoDynamic[${i}].logicRWDef`]: logicRWDef,
                [`listRWGapInfoDynamic[${i}].logicRWValueA`]: logicRWValueA,
                [`listRWGapInfoDynamic[${i}].logicRWValueB`]: logicRWValueB,
                [`listRWGapInfoDynamic[${i}].logicRWTaxitimeA`]: logicRWTaxitimeA,
                [`listRWGapInfoDynamic[${i}].logicRWTaxitimeB`]: logicRWTaxitimeB,
                [`listRWGapInfoDynamic[${i}].logicRWNameA`]: logicRWNameA,
                [`listRWGapInfoDynamic[${i}].logicRWNameB`]: logicRWNameB,
    ...
    const submitData = (value) => {
        RunwayDynamicEditFormData.toggleLoad(true);
        const opt = {
            url: ReqUrls.runwayDynamicUpdatelUrl + userId,
    ...
    const fetchConfigData = () => {
        RunwayDynamicEditFormData.toggleLoad(true);
        const opt = {
            url: ReqUrls.runwayDynamicDetailUrl + userId,
            method: "GET",
    ...
    const fetchSuccess = (data) => {
        ...
        RunwayDynamicEditFormData.updateConfigData(runwayData);
    };
...
)(observer(RunwayDynamicEditForm));
